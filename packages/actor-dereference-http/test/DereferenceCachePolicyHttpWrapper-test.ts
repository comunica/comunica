...
describe('DereferenceCachePolicyHttpWrapper', () => {
  ...
  let cachePolicy: ICachePolicy<Request>;
  ...
  beforeEach(() => {
    cachePolicy = {
      ...
      async revalidationHeaders(_newInput: Request): Promise<Headers> {
        return new Headers({ c: 'd' });
      },
      async satisfiesWithoutRevalidation(_input: Request): Promise<boolean> {
        return true;
      },
      storable(): boolean {
        return true;
  ...
  describe('satisfiesWithoutRevalidation', () => {
    it('is delegated to the cachePolicy', async() => {
      ...
      expect(cachePolicy.satisfiesWithoutRevalidation).toHaveBeenCalledWith(new Request(
        'http://localhost:8080',
        {
  ...
  describe('revalidationHeaders', () => {
    it('is delegated to the cachePolicy', async() => {
      ...
      expect(cachePolicy.revalidationHeaders).toHaveBeenCalledWith(new Request(
        'http://localhost:8080',
        {
          headers: new Headers({ accept: 'text/turtle;q=0.9' }),
          method: 'GET',
  ...
  describe('revalidatedPolicy', () => {
    it('is delegated to the cachePolicy', async() => {
      ...
      expect(cachePolicy.revalidatedPolicy).toHaveBeenCalledWith(new Request('http://localhost:8080', {
        headers: new Headers({ accept: 'text/turtle;q=0.9' }),
        method: 'GET',
