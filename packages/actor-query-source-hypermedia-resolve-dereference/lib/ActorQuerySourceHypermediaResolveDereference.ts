...
import type { IActionQuerySourceHypermediaResolve, IActorQuerySourceHypermediaResolveOutput, IActorQuerySourceHypermediaResolveArgs } from '@comunica/bus-query-source-hypermedia-resolve';
...
export class ActorQuerySourceHypermediaResolveDereference extends ActorQuerySourceHypermediaResolve {
  ...
  public constructor(args: IActorQuerySourceHypermediaResolveArgs) {
    super(args);
  }
  ...
  public async run(action: IActionQuerySourceHypermediaResolve): Promise<IActorQuerySourceHypermediaResolveOutput> {
    ...
    // TODO: don't track if first link (as that's already done in ContextPreprocess)
    const statisticDereferenceLinks = context.get(KeysStatistics.dereferencedLinks);
    if (statisticDereferenceLinks) {
      statisticDereferenceLinks.updateStatistic({ url: action.url, metadata }, source);
    }
