...
export class AlgebraTransformer extends ExprEval.TermTransformer {
  ...
  public async transformAlgebra(expr: Algebra.Expression): Promise<Expression> {
    if (Algebra.isKnownSub(expr, Algebra.ExpressionTypes.TERM)) {
    ...
    if (Algebra.isKnownSub(expr, Algebra.ExpressionTypes.NAMED)) {
      return await this.transformNamed(expr);
    }
    if (Algebra.isKnownSub(expr, Algebra.ExpressionTypes.EXISTENCE)) {
      return AlgebraTransformer.transformExistence(expr);
    }
    if (Algebra.isKnownSub(expr, Algebra.ExpressionTypes.AGGREGATE)) {
      return AlgebraTransformer.transformAggregate(expr);
    }
    if (Algebra.isKnownSub(expr, Algebra.ExpressionTypes.WILDCARD)) {
