    ...
    describe('for parsing', () => {
      ...
      it('should run', async () => {
        return expect(await arrayifyStream((await actor.run(
          { context, handle: { input, baseIRI: '' }, handleMediaType: 'text/html' }))
          .handle.quads)).toBeRdfIsomorphic([
      ...
      it('should run and delegate parsing errors', async () => {
        input = stringToStream(
        ...
        return expect(arrayifyStream((await actor.run(
          { context, handle: { input, baseIRI: '' },
            handleMediaType: 'text/html' })).handle.quads)).rejects
          .toThrow(new Error('Expected entity but got . on line 3.'));
      });

      it('should run with no script type but no output', async () => {
        input = stringToStream(
      ...
      it('should run with one N-Quads script', async () => {
        input = stringToStream(
          `<script type="application/n-quads">
            <http://example.org/f> <http://example.org/g> <http://example.org/h> .
            <http://example.org/f> <http://example.org/i> <http://example.org/j> .
      ...
      it('should run with two JSON-LD scripts', async () => {
        input = stringToStream(
          `<script type="application/ld+json">
          [{
              "@id": "http://example.org/a",
      ...
      it('should run with one JSON-LD script with a baseIRI', async () => {
        input = stringToStream(
          `<script type="application/ld+json">
          [{
              "@id": "a",
        ...
        return expect(await arrayifyStream((await actor.run(
          { context, handle: { input, baseIRI: 'http://ex.org' }, handleMediaType: 'text/html' }))
          .handle.quads)).toBeRdfIsomorphic([
            quad('http://ex.org/a', 'http://example.org/b', 'http://ex.org/c'),
            quad('http://ex.org/a', 'http://example.org/d', 'http://ex.org/e'),
          ]);
      });

      it('should run with one JSON-LD script and one text/plain script', async () => {
        input = stringToStream(
          `<script type="application/ld+json">
          [{
              "@id": "http://example.org/a",
      ...
      it('should run with two JSON-LD scripts and one N-Quads script', async () => {
        input = stringToStream(
          `<script type="application/ld+json">
          [{
              "@id": "http://example.org/a",
...
`);
      ...
      it('should run with real HTML input', async () => {
        input = stringToStream(
          `<!DOCTYPE html>
           <html>
            <body>
