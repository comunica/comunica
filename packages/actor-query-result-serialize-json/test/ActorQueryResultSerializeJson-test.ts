  ...
  describe('An ActorQueryResultSerializeJson instance', () => {
    ...
    describe('for serializing', () => {
      it('should test on application/json quads', async () => {
        ...
        await expect(actor
          .test({ handle: <any> { type: 'quads', quadStream: stream, context }, handleMediaType: 'application/json', context }))
          .resolves.toBeTruthy();
      ...
      it('should not test on N-Triples', async () => {
        ...
        await expect(actor.test(
          { handle: <any> { type: 'quads', quadStream: stream, context }, handleMediaType: 'application/n-triples', context },
        ))
          .rejects.toBeTruthy();
      ...
      it('should run on a bindings stream', async() => {
        expect(await stringifyStream((<any> (await actor.run(
          { handle: <any> { type: 'bindings', bindingsStream: bindingsStream(), context }, handleMediaType: 'application/json', context },
        ))).handle.data))
          .toEqual(
            `[
{"k1":"v1"},
...
`,
      ...
      it('should run on a quad stream', async() => {
        expect(await stringifyStream((<any> (await actor.run(
          { handle: <any> { type: 'quads', quadStream: quadStream(), context }, handleMediaType: 'application/json', context },
        ))).handle.data)).toEqual(
          `[
