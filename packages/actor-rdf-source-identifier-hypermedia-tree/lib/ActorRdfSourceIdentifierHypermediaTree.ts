...
/**
 * A comunica Hypermedia Tree RDF Source Identifier Actor.
 */
export class ActorRdfSourceIdentifierHypermediaTree extends ActorRdfSourceIdentifier {
  ...
  constructor(args: IActorRdfSourceIdentifierHypermediaTreeArgs) {
    super(args);
  }
  ...
  public async test(action: IActionRdfSourceIdentifier): Promise<IMediatorTypePriority> {
    ...
    throw new Error(`${action.sourceValue} is not a (Tree) hypermedia interface`);
  }

  public async run(action: IActionRdfSourceIdentifier): Promise<IActorRdfSourceIdentifierOutput> {
    return { sourceType: 'hypermedia', flags: { isTree: true } };
  }

}

export interface IActorRdfSourceIdentifierHypermediaTreeArgs
