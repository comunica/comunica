...
more streams that are all sorted on the same variable in a single pass, with a leapfrog join: every stream that is
behind the furthest one skips ahead to it, until all streams are at the same key, whose runs are then emitted as a
