...
describe('LeapfrogJoinIterator', () => {
  ...
  it('returns null when read after it has ended, and destroys its sources', async() => {
    ...
    const it = new LeapfrogJoinIterator(sources, compare);
  ...
  it('emits errors from its sources', async() => {
    ...
    const result = arrayifyStream(new LeapfrogJoinIterator([
      array([ bind(1, 'b', 'b1') ]),
      failing,
      array([ bind(1, 'd', 'd1') ]),
    ], compare));
    await new Promise(resolve => setImmediate(resolve));
    failing.fail(new Error('leapfrog source error'));
    await expect(result).rejects.toThrow('leapfrog source error');
  });
  ...
  it('waits for sources that have nothing buffered yet', async() => {
    ...
    const result = arrayifyStream(new LeapfrogJoinIterator(sources, compare));
  ...
  it('matches a nested loop join over randomized inputs with duplicate keys', async() => {
    ...
    const plain = await arrayifyStream(new LeapfrogJoinIterator(inputs.map(array), compare));
    expect(asMultiset(plain)).toEqual(asMultiset(expected));
    const seeking = await arrayifyStream(new LeapfrogJoinIterator(
  ...
  describe('with seekable sources', () => {
    ...
    it('reads through keys that interleave closely instead of skipping', async() => {
      ...
      await expect(arrayifyStream(new LeapfrogJoinIterator(sources, compare))).resolves.toHaveLength(100);
      for (const source of sources) {
  ...
  describe('seek', () => {
    ...
    it('skips every source ahead before anything was read', async() => {
      ...
      const it = new LeapfrogJoinIterator(inputs, compare);
      it.seek(bind(90, 'e', 'e'));
