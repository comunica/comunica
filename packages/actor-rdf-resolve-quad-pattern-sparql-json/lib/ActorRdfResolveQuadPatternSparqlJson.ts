...
export class ActorRdfResolveQuadPatternSparqlJson
  ...
  public async run(action: IActionRdfResolveQuadPattern): Promise<IActorRdfResolveQuadPatternOutput> {
    ...
    this.queryBindings(endpoint, countQuery, action.context)
      .then((bindingsStream: TBindingsStream) => new Promise(resolve => {
   ...
   * @return {Promise<TBindingsStream>} A promise resolving to a stream of bindings.
   */
  public async queryBindings(endpoint: string, query: string, context?: ActionContext): Promise<TBindingsStream> {
