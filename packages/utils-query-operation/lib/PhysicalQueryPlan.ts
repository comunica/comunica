...
/**
 * Group the sub-operations that will be executed within the returned context under a single node of
 * the physical query plan, so that repeated evaluations are summarized rather than listed next to
 * the actual inputs.
 *
 * Calling this again for the same repetition yields the same group, so callers need not remember it.
