...
import { BindingsRaw } from '../lib/BindingsRaw';
import 'jest-rdf';

const DF = new DataFactory();
const contextMergeHandlers = {};
describe('BindingsRaw', () => {
  describe('without context', () => {
    let bindings: BindingsRaw;

    beforeEach(() => {
      bindings = new BindingsRaw(DF, Map<string, RDF.Term>([
        [ 'a', DF.namedNode('ex:a') ],
        [ 'b', DF.namedNode('ex:b') ],
        [ 'c', DF.namedNode('ex:c') ],
    ...
    describe('equals', () => {
      ...
      it('should be false for empty bindings', () => {
        expect(bindings.equals(new BindingsRaw(DF, Map<string, RDF.Term>([]), { contextMergeHandlers }))).toBeFalsy();
      });

      it('should be false for bindings with fewer keys', () => {
        expect(bindings.equals(new BindingsRaw(DF, Map<string, RDF.Term>([
          [ 'a', DF.namedNode('ex:a') ],
          [ 'b', DF.namedNode('ex:b') ],
    ...
    describe('merge', () => {
      describe('with RDF.Bindings', () => {
        it('should merge distinct bindings', () => {
          const OriginalBindingsRaw = new BindingsRaw(DF, Map<string, RDF.Term>([
            [ 'd', DF.namedNode('ex:d') ],
            [ 'e', DF.namedNode('ex:e') ],
            [ 'f', DF.namedNode('ex:f') ],
          ]), { contextMergeHandlers });
          const bindingsOther: RDF.Bindings = <any> {
            [Symbol.iterator]: OriginalBindingsRaw[Symbol.iterator].bind(OriginalBindingsRaw),
            equals: OriginalBindingsRaw.equals.bind(OriginalBindingsRaw),
          };
          expect(bindingsOther instanceof BindingsRaw).toBeFalsy();

          const bindingsNew: BindingsRaw = bindings.merge(bindingsOther)!;
      ...
      describe('with native BindingsRaw', () => {
        it('should merge distinct bindings', () => {
          const bindingsOther = new BindingsRaw(DF, Map<string, RDF.Term>([
            [ 'd', DF.namedNode('ex:d') ],
            [ 'e', DF.namedNode('ex:e') ],
            [ 'f', DF.namedNode('ex:f') ],
          ]), { contextMergeHandlers });
    ...
    describe('mergeWith', () => {
      describe('with RDF.Bindings', () => {
        it('should merge distinct bindings', () => {
          ...
          const cb = jest.fn();
          const bindingsNew: BindingsRaw = bindings.mergeWith(cb, bindingsOther);
          expect(bindingsNew).toBeDefined();
          expect(bindingsNew).not.toBe(bindings);
          expect(bindingsNew.size).toBe(6);
    ...
    describe('toString', () => {
      it('should stringify empty bindings', () => {
        expect(new BindingsRaw(DF, Map<string, RDF.Term>(), { contextMergeHandlers }).toString()).toBe(`{}`);
      });
  ...
  describe('with context', () => {
    let bindings: BindingsRaw;
    let bindingsNoContext: BindingsRaw;

    beforeEach(() => {
      bindings = new BindingsRaw(
        DF,
        Map<string, RDF.Term>([
          [ 'a', DF.namedNode('ex:a') ],
          [ 'b', DF.namedNode('ex:b') ],
        ...
        {
          contextMergeHandlers: { source: new SetUnionBindingsRawContextMergeHandler() },
          context: new ActionContext({ source: [ 'ex:S1', 'ex:S2', 'ex:S3' ]}),
        },
      );
      bindingsNoContext = new BindingsRaw(
        DF,
        Map<string, RDF.Term>([
          [ 'a', DF.namedNode('ex:a') ],
          [ 'b', DF.namedNode('ex:b') ],
  ...
  describe('with context that is merged', () => {
    ...
    it('should merge binding context according to mergehandler in mergeWith', () => {
      const bindingsOther = new BindingsRaw(
        DF,
        Map<string, RDF.Term>([
          [ 'd', DF.namedNode('ex:d') ],
          [ 'a', DF.namedNode('ex:a') ],
    ...
    it('should merge own binding context with extra key by adding to result context without change', () => {
      const bindingsExtraKey = new BindingsRaw(
        DF,
        Map<string, RDF.Term>([
          [ 'd', DF.namedNode('ex:d') ],
          [ 'a', DF.namedNode('ex:a') ],
      ...
      const bindingsNew: BindingsRaw = bindingsExtraKey.merge(bindings)!;
      expect(bindingsNew).toBeDefined();
    ...
    it('should merge other binding context with extra key by adding to result context without change', () => {
      ...
      const bindingsNew: BindingsRaw = bindings.merge(bindingsExtraKey)!;
      expect(bindingsNew).toBeDefined();
      expect(bindingsNew.getContext())
    ...
    it(`should merge remove all binding context entries that occur in both contexts but dont have a mergehandler`, () => {
      const bindingsNoMergeHandler = new BindingsRaw(
        DF,
        Map<string, RDF.Term>([
          [ 'd', DF.namedNode('ex:d') ],
          [ 'a', DF.namedNode('ex:a') ],
      ...
      const bindingsNew: BindingsRaw = bindingsNoMergeHandler.merge(bindings)!;
      expect(bindingsNew).toBeDefined();
    ...
    it('should merge with undefined context', () => {
      const bindingsNoContextOther = new BindingsRaw(
        DF,
        Map<string, RDF.Term>([
          [ 'a', DF.namedNode('ex:a') ],
          [ 'b', DF.namedNode('ex:b') ],
    ...
    describe('calling merge twice on same binding should give correct results', () => {
      let bindingsOther1: BindingsRaw;
      let bindingsOther2: BindingsRaw;
      beforeEach(() => {
        bindingsOther1 = new BindingsRaw(
          DF,
          Map<string, RDF.Term>([
            [ 'd', DF.namedNode('ex:d') ],
            [ 'a', DF.namedNode('ex:a') ],
        ...
        bindingsOther2 = new BindingsRaw(
          DF,
          Map<string, RDF.Term>([
            [ 'd', DF.namedNode('ex:d') ],
            [ 'a', DF.namedNode('ex:a') ],
      ...
      it('calling merge twice with different bindings should give correct results', () => {
        const bindingsNew1: BindingsRaw = bindings.merge(bindingsOther1)!;
        expect(bindingsNew1).toBeDefined();
        expect(bindingsNew1.getContext()).toEqual(new ActionContext({ source: [ 'ex:S1', 'ex:S2', 'ex:S3', 'ex:S5' ]}));
        const bindingsNew2: BindingsRaw = bindings.merge(bindingsOther2)!;
      ...
      it('calling mergeWith twice with different bindings should give correct results', () => {
        const cb = jest.fn();
        const bindingsNew1: BindingsRaw = bindings.mergeWith(cb, bindingsOther1);
        expect(bindingsNew1).toBeDefined();
        expect(bindingsNew1.getContext()).toEqual(new ActionContext({ source: [ 'ex:S1', 'ex:S2', 'ex:S3', 'ex:S5' ]}));
        const bindingsNew2: BindingsRaw = bindings.mergeWith(cb, bindingsOther2);
        expect(bindingsNew2).toBeDefined();
        expect(bindingsNew2.getContext()).toEqual(new ActionContext({ source: [ 'ex:S1', 'ex:S2', 'ex:S3', 'ex:S9' ]}));
      });
    });
